...
Hız: run 16GB bellekte çalışır (apify_memory ile ayarlanabilir).
"""
...
def find_creators(cfg: dict) -> List[dict]:
 ...
 # HIZ: run belleği (MB). Varsayılan 16GB = 16384. Daha fazla RAM = daha hızlı.
 ...
 # run POST: memory query paramı ile 16GB'de çalıştır (hız için).
 run_url = f"{APIFY_BASE}/acts/{actor}/runs"
 run_params = {"token": token, "memory": apify_memory}
 r = requests.post(run_url, params=run_params, json=actor_input, timeout=30)
 if r.status_code >= 400:
 # Bazı actor'lar yüksek belleğe izin vermezse belleksiz tekrar dene.
